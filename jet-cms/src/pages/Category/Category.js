...
export default function Videos() {
  ...
  useEffect(() => {
    ( async ()=>{
      const videos = await axios.get('http://localhost:3000/api/categories');
  ...
  function addNewVideo(e) {
    ...
    axios.post('http://localhost:3000/api/categories', {title, link, img});
    setAddVideo(false);
  }
  ...
  function updateVideo(e) {
    e.preventDefault();
    const [title, link, img] = [e.target.video_title.value, e.target.video_link.value, e.target.video_img.value];
    axios.patch(`http://localhost:3000/api/categories/${videoId}`, {title, link, img});
    setEditVideo(false);
  }
